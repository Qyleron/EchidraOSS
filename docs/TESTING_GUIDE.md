...
|---|---|---|
| Sessions | `/dashboard/sessions` | Table is a flat, one-row-per-session log (no grouping/collapsing), newest first; clicking a row opens a slide-over Session Inspector drawer (not an inline row) with its command history and, once loaded, a "View in Intelligence" badge if that session was aggregated into an Intelligence issue — the Inspector doesn't repeat fields already on the row (Source IP, Persona, Start, Duration, Risk); the date-range control sits next to Export CSV/XLSX in the header — the underlying date fields are still native `<input type="date">`, but their calendar chrome is replaced by a custom-rendered popup grid, not the OS's native calendar UI; search box has a magnifying-glass icon |
